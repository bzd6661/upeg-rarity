    ...
    <ScrollArea className="h-full">
      <div className="mx-auto max-w-7xl space-y-8 p-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          ...
          {trichrome > 0 && (
            <StatCard
              label="Trichrome (3-color)"
              value={trichrome.toLocaleString()}
            />
          )}
          ...
          return (
            <Card key={cat}>
              <CardHeader className="pb-2 pt-3">
                <CardTitle className="text-sm font-semibold capitalize tracking-wide text-zinc-300">
                  {cat}
                </CardTitle>
              </CardHeader>
              <CardContent className="pb-4">
                <ResponsiveContainer
                  width="100%"
                  height={Math.max(180, data.length * 24)}
                >
