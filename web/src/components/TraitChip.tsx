...
export function TraitChip({ label, value, frequency, derived }: Props) {
  ...
  const baseClass = "inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs";
  ...
  return (
    <span className={`${baseClass} ${colorClass}`}>
      ...
      {rarityHint && (
        <span className={derived ? "text-emerald-500" : "text-zinc-400"}>({rarityHint})</span>
      )}
    </span>
  );
}
